...
def tt_parse(args=None):
    ...
    parent_parser.add_argument(
        '--pr-workflow',
        action='store_true',
        help='Migration flag to ease transition to a new workflow',
        default=False,
        dest='pr_workflow',
    )
    ...
    poll_group.add_argument(
        ...
        dest='poll',
        action='store_true',
        help='Poll the status until it is set to success or failure (default)',
    )
    poll_group.add_argument(
        '--no-poll',
        dest='poll',
        action='store_false',
        help='Do not continue to poll the status, just fire and forget',
    )
    ...
    workon_parser.add_argument(
        ...
        dest='body',
        type=str,
        help='Optional body (issue comment) for the new issue',
    )

    assign_group = workon_parser.add_mutually_exclusive_group()
    assign_group.add_argument(
        '--assign', dest='assignee', action='store_true', help='Assign @me to the issue (default)'
    )
    assign_group.add_argument(
        '--no-assign',
        dest='assignee',
        action='store_false',
        help='Do not assign anybody to the issue',
    )
    ...
    deliver_parser = subparsers.add_parser(
        'deliver',
        parents=[parent_parser, poll_parser],
    ...
    deliver_parser.add_argument(
        ...
        dest='delete_branch',
        default=False,
        help='Delete branch after the PR is merged. Only supported with the --pr-workflow flag.',
    )
    ...
    semver_bump_parser = semver_sub_parser.add_parser(
        'bump',
        parents=[parent_parser],
        help='Bumps the current version of the repository in semantic versioning format',
    )
    semver_bump_level_group = semver_bump_parser.add_mutually_exclusive_group(required=True)
    semver_bump_level_group.add_argument(
        '--major',
        dest='level',
        help='Bump the major version, breaking change',
        action='store_const',
        const='major',
    )
    semver_bump_level_group.add_argument(
        '--minor',
        dest='level',
        help='Bump the minor version, new feature, non-breaking change',
        action='store_const',
        const='minor',
    )
    semver_bump_level_group.add_argument(
        '--patch',
        dest='level',
        help='Bump the patch version, bug fix or rework, non-breaking change',
        action='store_const',
        const='patch',
    )
    semver_bump_level_group.add_argument(
        ...
        '--pre',
        dest='level',
        help='Bump the prerelease version',
        action='store_const',
        const='prerelease',
    )
    semver_bump_level_group.add_argument(
        '--build', dest='level', help='Bump the build version', action='store_const', const='build'
    )
    semver_bump_parser.add_argument(
        '-m',
        '--message',
        type=str,
        help='Additional message for the annotated tag',
        required=False,
        default=None,
    )
    semver_bump_parser.add_argument(
        '--prefix',
        type=str,
        help='Prefix to prepend the tag valid for both releases and prereleases',
        required=False,
        default=None,
    )
    semver_bump_parser.add_argument(
        ...
        dest='include_sha',
        action='store_false',
        help='Do not include git SHA in build number (only valid with --build)',
        required=False,
        default=True,
    )
    ...
    run_group.add_argument(
        '--no-run', dest='run', action='store_false', help='Print the command without executing it'
    )
    semver_bump_parser.set_defaults(run=True, exclusive_groups=['bump'])

    # List command with filter options
    semver_list_parser = semver_sub_parser.add_parser(
        'list',
        parents=[parent_parser],
        help='Lists the version tags in the repository in semantic versioning format and sort order',
    )
    ...
    list_filter_group.add_argument(
        ...
        action='store_const',
        const='release',
        help='Show only release versions',
    )
    list_filter_group.add_argument(
        '--prerelease',
        '--pre',
        ...
        action='store_const',
        const='prerelease',
        help='Show only prerelease versions',
    )
    list_filter_group.add_argument(
        '--other',
        dest='filter_type',
        action='store_const',
        const='other',
        help='Show only non-semantic version tags',
    )
    list_filter_group.add_argument(
        '--all',
        dest='filter_type',
        action='store_const',
        const='all',
        help='Show all version tags (default)',
    )
    semver_list_parser.add_argument(
        '--sha', action='store_true', help='Show commit SHAs for each version tag', default=False
    )
    ...
    if (
        hasattr(args, 'include_sha')
        and args.include_sha is False
        and (not hasattr(args, 'level') or args.level != 'build')
    ):
    ...
    if args.command == 'deliver' and (not args.pr_workflow and args.delete_branch):
        deliver_parser.error(
            '🛑 The --delete-branch flag can only be used with the --pr-workflow flag'
        )
