...
async def deliver(*, delete_branch: bool):
    ...
    if remote_dev_branch_tip_hash != current_branch_tip_hash:
        logger.debug('branch %s is not up to date with its remote %s/%s', current_branch, remote, current_branch)
        print(f'Branch {current_branch} is not up to date with its remote. You may have unpushed commits on your local branch. Align your local branch with its remote before delivering.', file=sys.stderr)
        sys.exit(1)
