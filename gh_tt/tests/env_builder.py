...
class IntegrationEnv:
    ...
    def require_owner(self) -> Self:
        ...
        async def create():
            ...
            if not is_gh_actions:
                ...
                assert result.return_code == 0, 'gh CLI is not authenticated. Run: gh auth login'

                result = await shell.run(['gh', 'org', 'list'])
                orgs = result.stdout
                assert 'gh-tt-qa' in orgs, (
                    'You must have access to the gh-tt-qa organization to trigger integration tests locally. Contact a maintainer of gh-tt for access.'
                )

                result = await shell.run(['gh', 'extension', 'list'])
                extensions = result.stdout

                assert 'thetechcollective/gh-tt' not in extensions, (
                ...
                assert 'gh tt' in extensions, (
                    'gh tt is not installed. Install with\ngh extension install .'
                )
            ...
            if qa_org_id is not None:
                result = await shell.run(
                    [
                        ...
                        'graphql',
                        '-f',
                        f'query=query {{ node(id: "{qa_org_id}") {{ ... on Organization {{ name }} }} }}',
                    ]
                )
    ...
    def create_repo(self) -> Self:
        ...
        async def create():
            ...
            result = await shell.run(['gh', 'repo', 'create', repo_name, '--private'])
            self.repo_url = result.stdout

        async def cleanup():
            if self.repo_url:
                await shell.run(
                    ['gh', 'repo', 'delete', self.repo_url, '--yes'], die_on_error=False
                )
    ...
    def create_issue(self, title: str | None = None) -> Self:
        ...
        async def create():
            ...
            result = await shell.run(
                ['gh', 'issue', 'create', '-R', self.repo_url, '--title', issue_title, '--body', '']
            )
    ...
    def create_local_clone(self, default_branch: str = 'main') -> Self:
        ...
        async def create():
            ...
            if self.repo_url:
                await shell.run(
                    ['git', 'remote', 'add', 'origin', self.repo_url], cwd=self.local_repo
                )

                # Push initial commit so remote has a default branch
                await shell.run(
                    ['git', 'push', '-u', 'origin', default_branch], cwd=self.local_repo
                )
    ...
    def create_gh_project(self) -> Self:
        async def create():
            ...
            project_number = await shell.run(
                [
                    'gh',
                    'project',
                    'create',
        ...
        async def create():
            ...
            with Path.open(self.local_repo / CONFIG_FILE_NAME, 'w') as f:
                f.write(
                    json.dumps(
                        {
                            'workon': {'status': workon_status_value},
                            'project': {'owner': self.owner, 'number': self.project_number},
                        }
                    )
                )
