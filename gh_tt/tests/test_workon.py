...
async def test_workon_basic_success():
    async with (IntegrationEnv()
        ...
        pr_data = await shell.run(['gh', 'pr', 'view', branch_name, '-R', env.repo_url, '--json', 'number,isDraft,body'])
...
async def test_workon_reuses_remote_branch():
    async with (IntegrationEnv()
        ...
        await shell.run(['git', 'commit', '--allow-empty', '-m', 'marker commit'], cwd=env.local_repo)
...
async def test_workon_commits_empty_with_pending_changes():
    async with (IntegrationEnv()
        ...
        await shell.run(['echo', '"text"', '>', 'added.txt', '&&', 'git add added.txt'], cwd=env.local_repo)
        ...
        result = await shell.run(['git', 'rev-list', '--count', f'main..{branch_name}'], cwd=env.local_repo)
        assert int(result.stdout) == 1

        # Verify new branch's commit is empty
        result = await shell.run(['git', 'diff-tree', '--no-commit-id', '--name-only', '-r', 'HEAD'], cwd=env.local_repo)
