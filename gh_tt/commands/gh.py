...
async def get_project(project_number: int, project_owner: str) -> Project:
    result = await shell.run(['gh', 'project', 'view', str(project_number), '--owner', project_owner, '--format', 'json'])
...
async def add_item_to_project(project_number: int, project_owner: str, item_url: str) -> ProjectItem:
    result = await shell.run(['gh', 'project', 'item-add', str(project_number), '--owner', project_owner, '--url', item_url, '--format', 'json'])
...
async def update_project_item_status(project_id: str, project_number: int, project_owner: str, item_id: str, status_value: str):
    status_field_name = 'Status'
    
    result = await shell.run(['gh', 'project', 'field-list', str(project_number), '--owner', project_owner, '--format', 'json', '--jq', f'.fields[] | select(.name == "{status_field_name}")'])

    project_status_field = ProjectStatusField(**json.loads(result.stdout))

    status_option_id = next(option.option_id for option in project_status_field.options if option.name == status_value)
